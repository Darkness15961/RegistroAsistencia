    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        <div 
          ...
          :class="getAreaStyle(grupo.area.nombre_area).gradient"
        >
          <i :class="[getAreaStyle(grupo.area.nombre_area).icon, 'text-white text-xl']"></i>
        </div>
        <h3 
...
// Función para mostrar un nombre de grupo legible
const getNombreGrupo = (grupo) => {
  if (grupo.grado || grupo.nivel) {
    return `${grupo.nivel || ''} ${grupo.grado || ''} "${grupo.seccion || ''}"`.trim()
  }
  // Si no tiene grado/nivel, usa el nombre del área como fallback (ej. "Administración")
  return grupo.area.nombre_area
}

// Lógica de iconos y gradientes (traída de Áreas)
const styleCatalog = {
