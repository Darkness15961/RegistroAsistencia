        ...
        <table class="w-full">
          <thead :class="theme('tableHeader').value">
            <tr class="border-b" :class="isDark ? 'border-white/20' : 'border-gray-200'">
              ...
              <th class="text-left p-4 font-bold text-sm uppercase" :class="theme('cardSubtitle').value">Cargo</th>
            ...
            >
              <td class="p-4">
                <div class="flex items-center gap-3">
                  <div class="w-10 h-10 rounded-xl object-cover shadow-lg bg-gradient-to-br from-blue-400 to-cyan-500 text-white flex items-center justify-center font-bold">
                    {{ empleado.nombre_completo.substring(0, 2).toUpperCase() }}
                  </div>
                  <div>
                    <span class="font-semibold" :class="theme('cardTitle').value">{{ empleado.nombre_completo }}</span>
    ...
    <div class="grid gap-4 md:hidden">
      ...
      >
        <div class="flex items-start justify-between gap-3 mb-4">
          <div class="flex items-center gap-3">
            <div class="w-12 h-12 rounded-2xl object-cover shadow-lg bg-gradient-to-br from-blue-400 to-cyan-500 text-white flex items-center justify-center font-bold">
              {{ empleado.nombre_completo.substring(0, 2).toUpperCase() }}
            </div>
            <div>
        ...
        <div class="flex flex-col gap-3 pt-3 border-t" :class="isDark ? 'border-white/10' : 'border-gray-200'">
            <div class="flex items-center gap-2">
              <i class="fas fa-briefcase text-sm w-4 text-center" :class="theme('cardSubtitle').value"></i>
              <span class="font-medium text-sm" :class="theme('cardTitle').value">{{ empleado.cargo_grado }}</span>
            </div>
            <div class="flex items-center gap-2">
