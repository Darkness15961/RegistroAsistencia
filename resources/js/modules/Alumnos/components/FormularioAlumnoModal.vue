...
const previewDataUrl = ref('')       // Usado para mostrar en <img>
const currentDescriptor = ref(null)  // Almacena el descriptor (Array)
let streamTracks = []
...
const capturarFoto = async () => {
  ...
  clearCapture() // Limpiar captura anterior
  ...
  capturedImage.value = dataUrl // Este es el image_base64
...
const analizarRostro = async () => {
  ...
  try {
    ...
    // Guardamos el descriptor (convertido a Array para JSON)
    currentDescriptor.value = Array.from(detection.descriptor) 

    // Dibujar los puntos (landmarks) en el canvas de preview
    ...
    faceapi.draw.drawFaceLandmarks(canvas, resizedDetection) // Dibujar puntos
...
watch(() => props.alumno, (nuevo) => {
  if (nuevo) {
    ...
    // Cargar imagen existente (si tu API la devuelve)
    if (nuevo.image_url) {
       previewDataUrl.value = nuevo.image_url
       capturedImage.value = nuevo.image_url
    }
     // Limpiar datos de IA al cambiar de alumno
    clearCapture()
  } else {
    // Resetear formulario
    form.value = { 
...
/* --------------------- Guardado (Lógica Principal Corregida) --------------------- */
const guardar = async () => {
  loading.value = true
  ...
  try {
    ...
    if (errorData && errorData.errors) {
        ...
        alert(errorData?.message || err?.message || 'Error al guardar alumno')
    }
